...
// Récupérer les informations du highlight
$highlight = fetchOne("SELECT * FROM community_highlights WHERE id = ?", [$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($highlight['title'])) {
        ...
        if (isset($_POST['image_type']) && $_POST['image_type'] === 'upload' && isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
            ...
            if (!file_exists(dirname(dirname(__DIR__)) . '/uploads')) {
                mkdir(dirname(dirname(__DIR__)) . '/uploads', 0775, true);
            }
            if (!file_exists(dirname(dirname(__DIR__)) . '/uploads/highlights')) {
                mkdir(dirname(dirname(__DIR__)) . '/uploads/highlights', 0775, true);
            }
        ...
        } elseif (isset($_POST['image_type']) && $_POST['image_type'] === 'url' && isset($_POST['image_url']) && !empty($_POST['image_url'])) {
            // Utilisation d'une URL externe
            $highlight['image_url'] = sanitizeInput($_POST['image_url']);
        }
        // Si 'keep' est sélectionné, on garde l'image actuelle
        
        if (empty($error)) {
            // Mise à jour du highlight dans la base de données
            $updateResult = updateData('community_highlights', $highlight, ['id' => $id]);
            
            if ($updateResult) {
                $success = 'Le highlight a été mis à jour avec succès.';
                
                // Récupérer les informations mises à jour
                $highlight = fetchOne("SELECT * FROM community_highlights WHERE id = ?", [$id]);
            } else {
                $error = 'Une erreur est survenue lors de la mise à jour du highlight.';
            }
        }
    }
}
...
<head>
    ...
    <title>Modifier un highlight - StachZer Admin</title>
        ...
        <div class="flex-grow overflow-auto">
            <div class="p-6">
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-3xl font-bold">Modifier un highlight</h1>
                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <?php if (!empty($highlight['image_url'])): ?>
                                        <div class="mb-2">
                                            <label class="inline-flex items-center">
                                                <input type="radio" name="image_type" value="keep" class="form-radio text-pink-300" checked>
                                                <span class="ml-2">Conserver l'image actuelle</span>
                                            </label>
                                        </div>
                                    <?php endif; ?>
                                    ...
                                    <div class="mb-2">
                                        <label class="inline-flex items-center">
                                            <input type="radio" name="image_type" value="upload" class="form-radio text-pink-300" <?php echo empty($highlight['image_url']) ? 'checked' : ''; ?>>
                                            <span class="ml-2">Télécharger une nouvelle image</span>
                                        </label>
                                    </div>
                                ...
                                <div class="mt-6">
                                    ...
                                    <div class="bg-gray-900 p-4 rounded-lg">
                                        <div class="community-card preview-card relative h-56 rounded-lg overflow-hidden border border-gray-700">
                                            <div id="preview-image" class="w-full h-full bg-gray-700 flex items-center justify-center" style="background-image: url('<?php echo htmlspecialchars($highlight['image_url']); ?>'); background-size: cover; background-position: center;">
                                                <?php if (empty($highlight['image_url'])): ?>
                                            ...
                                            <div class="absolute bottom-0 left-0 right-0 p-4 z-10">
                                                <div class="flex items-center mb-2">
                                                    <div id="preview-initials" class="w-8 h-8 rounded-full mr-2 flex items-center justify-center text-xs font-bold" style="background-color: <?php echo htmlspecialchars($highlight['color_code']); ?>">
