...
$weapon = fetchOne("SELECT * FROM weapons WHERE id = ?", [$id]);
...
$selectedTags = fetchAll("
    SELECT tag_id FROM weapon_tag_relations WHERE weapon_id = ?
", [$id]);
...
if (empty($weapon['name'])) {
    ...
    if (isset($_POST['image_type']) && $_POST['image_type'] === 'upload' && isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
            ...
            $uploadResult = uploadImage($_FILES['image'], 'weapons');
        ...
        if (empty($error)) {
            // Mise à jour de l'arme dans la base de données
            $updateResult = updateData('weapons', $weapon, ['id' => $id]);
            ...
            if ($updateResult) {
                ...
                $success = 'L\'arme a été mise à jour avec succès.';
                $selectedTagIds = isset($_POST['tags']) ? $_POST['tags'] : [];
                
                // Récupérer les informations mises à jour
                $weapon = fetchOne("SELECT * FROM weapons WHERE id = ?", [$id]);
            } else {
                $error = 'Une erreur est survenue lors de la mise à jour de l\'arme.';
            }
        }
    }
}
...
<head>
    ...
    <title>Modifier une arme - StachZer Admin</title>
        ...
        <div class="flex-grow overflow-auto">
            <div class="p-6">
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-3xl font-bold">Modifier une arme</h1>
                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="mb-4">
                                    ...
                                    <?php if (!empty($weapon['image_url'])): ?>
                                        <div class="mb-4 bg-gray-700 rounded-lg overflow-hidden">
                                            <img src="<?php echo htmlspecialchars($weapon['image_url']); ?>" alt="<?php echo htmlspecialchars($weapon['name']); ?>" class="w-full h-48 object-cover">
                                        </div>
                                    <?php endif; ?>
                                    
                                    <div class="mb-2">
                                        <label class="inline-flex items-center">
                                            <input type="radio" name="image_type" value="keep" class="form-radio text-pink-300" <?php echo !empty($weapon['image_url']) ? 'checked' : ''; ?>>
                                    ...
                                    <div class="mb-2">
                                        <label class="inline-flex items-center">
                                            <input type="radio" name="image_type" value="upload" class="form-radio text-pink-300" <?php echo empty($weapon['image_url']) ? 'checked' : ''; ?>>
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        <?php foreach ($weaponTags as $tag): ?>
                                            <label class="inline-flex items-center bg-gray-700 p-2 rounded">
                                                <input type="checkbox" name="tags[]" value="<?php echo $tag['id']; ?>" class="form-checkbox text-pink-300" <?php echo in_array($tag['id'], $selectedTagIds) ? 'checked' : ''; ?>>
                            ...
                            <div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="range" id="mobility" name="mobility" min="0" max="100" value="<?php echo $weapon['mobility']; ?>" class="w-full" oninput="document.querySelector('label[for=\'mobility\']').textContent = 'Mobilité (' + this.value + '%)'">
                                </div>
